...
##2022_0614 JH : label 정렬 방법 오류 수정
try:
...
__appname__ = 'Labeling_V2022_0614'
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    # JH: self.open_next_image() 이미지 초기화 문제 때문에 사용중지
    ...
    def _save_file(self, annotation_file_path):
        if annotation_file_path and self.save_labels(annotation_file_path):
            ...
            # jh : xml 삭제 후 복사
            def delete_xml(xml_directory):
                [os.remove(f) for f in glob.glob(xml_directory + "*.xml")]

            def copy_xml(filenames, xml_directory):
            ...
            try:
                delete_xml(xml_directory)
                copy_xml(filenames, xml_directory)
            except:
                pass
    ...
    # jh: xml 삭제 추가, 삭제시 다음 이미지로 전환 추가
    def delete_image(self):
        ...
        if delete_path is not None:
            ...
            self.cur_img_idx -= 1
            self.img_count -= 1
            itemp = self.cur_img_idx

            if os.path.exists(delete_path):
                os.remove(delete_path)
                if (os.path.exists(delete_path[:-3] + 'xml')):
                    os.remove(delete_path[:-3] + 'xml')
            self.import_dir_images(self.last_open_dir)
            self.load_file(self.m_img_list[itemp])
...
def get_main_app(argv=[]):
    ...
    # JH : 라벨 개수 파악 위해 추가
    def count_label():
        # 모니터 해상도 이용 하여 label count 표기
        user32 = ctypes.windll.user32
        screensize = user32.GetSystemMetrics(0), user32.GetSystemMetrics(1)
        label_xposition = int(screensize[0] - 100)
        ...
        df3 = df2.dropna()
        str_temp_list = []
        ## JH : 2022_0614 한 xml 에 문자,숫자 섞였을 때 숫자를 문자 label로 저장하여 label 종류 증가하는 버그 수정
        temp_list = []
        ...
        df4 = df4[0].astype('float')

        if (len(str_temp_list) > 0):    # 2022_1101 jh : label counts 표기 안되는 버그 수정
            df5 = pd.DataFrame(str_temp_list)
            df5 = df5[0]
        list_of_label_index = df4.unique()
        ok_label_counts = len(df2[5]) - len(df2[6])
        win.text_label_ok = QLabel(win)
        win.text_label_ok.move(label_xposition, label_yposition)
        print_text_ok = 'ok_label : ' + str(ok_label_counts) + ' EA'
        win.text_label_ok.setText(print_text_ok)
        # end line
        win.text_label_f = QLabel(win)
        win.text_label_f.move(label_xposition, label_yposition + 20)
        print_text_f = '---------------'
        win.text_label_f.setText(print_text_f)

        # 문자열과 혼합시 sorting 불가 list_of_label_index.sort()
        label_counts = len(list_of_label_index)

        # JH: label_name 이 숫자로 이루어진 것들 우선 정렬
        Num_filtered_List = list(filter(lambda x: (type(x) == float) | (type(x) == np.float64), list_of_label_index))
        Num_filtered_List.sort()
        Num_label_counts = len(Num_filtered_List)

        Str_filtered_List = list(filter(lambda x: (type(x) == str), str_temp_list))
        Str_filtered_List.sort()
        Str_label_counts = len(Str_filtered_List)

        if (Str_label_counts != 0):
            for j in range(Str_label_counts):
                label_name = Str_filtered_List[j]
                label_data = (sum(df5 == Str_filtered_List[j]))
                win.text_label = QLabel(win)
                win.text_label.move(label_xposition, label_yposition - (20 * (Str_label_counts - j)))
                print_text = str(label_name) + ' : ' + str(label_data) + ' EA'
                win.text_label.setText(print_text)

        for l in range(Num_label_counts + 1):
            if (l == Num_label_counts):  # start line
                win.text_label = QLabel(win)
                win.text_label.move(label_xposition, label_yposition - (20 * (label_counts + 2)))
                print_text = '---------------'
                win.text_label.setText(print_text)
                break
            else:
                label_data = (sum(df4 == Num_filtered_List[l]))
                label_name = str(int(Num_filtered_List[l]))
                win.text_label = QLabel(win)
                win.text_label.move(label_xposition, label_yposition - (20 * (label_counts - l + 1)))
                print_text = str(label_name) + ' : ' + str(label_data) + ' EA'
                win.text_label.setText(print_text)
